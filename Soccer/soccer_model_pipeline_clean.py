#!/usr/bin/env python3
"""
Clean + optimized European club soccer retrain pipeline.

- Uses mirrored rows (team A / team B swap) for symmetry.
- Trains 5 model variants + isotonic calibration.
- Saves artifacts to a fresh output directory.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Clean + optimized soccer model pipeline")
    parser.add_argument("--data-dir", default="Soccer\\soccer_data")
    parser.add_argument("--outdir", default="Soccer\\soccer_outputs_clean")
    ...
    if not os.path.exists(matches_path):
        raise FileNotFoundError(f"Missing {matches_path}. Run scraper first.")
    ...
    meta = {
        ...
        "scope": "European club competitions only (incl. merged CL/EL)",
    }
    meta_path = os.path.join(args.outdir, "soccer_artifacts_meta.json")
